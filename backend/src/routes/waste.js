...
router.post('/', async (req, res) => {
    try {
        const { reporter, location, wasteType } = req.body;

        if (!reporter || !location || !wasteType) {
            return res.status(400).json({ message: "Missing required fields" });
        }

        // Assign blockchainId safely
...
// PATCH by blockchainId
router.patch('/blockchain/:blockchainId', async (req, res) => {
    try {
        const report = await WasteReport.findOne({ blockchainId: req.params.blockchainId });
        if (!report) {
            return res.status(404).json({ message: 'Report not found' });
        }
